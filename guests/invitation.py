...
def get_invitation_context(party):
    return {
        ...
        'main_color': '#fff3e8',
        'font_color': '#666666',
        'page_title': "Kelsey and Dain - You're Invited!",
...
def send_invitation_email(party, test_only=False, recipients=None):
    ...
    if not recipients:
        LOG.warn('===== WARNING: no valid email addresses found for {} ====='.format(party))
        return

    context = get_invitation_context(party)
    context['email_mode'] = True
    template_html = render_to_string(INVITATION_TEMPLATE, context=context)
    template_text = "You're invited to Kelsey and Dain's wedding. To view this invitation, visit {} in any browser.".format(
    ...
    LOG.info('sending invitation to {} ({})'.format(party.name, ', '.join(recipients)))
    if not test_only:
        msg.send()
